...
insula::height_map::cli_options()
{
	...
	opts.add_options()
		("cell-size",boost::program_options::value<graphics::scalar>()->default_value(static_cast<graphics::scalar>(5.0)),"Size dimensions of a single grid cell (it will be quadratic, hence only one value)")
		("height-scaling",boost::program_options::value<graphics::scalar>()->default_value(static_cast<graphics::scalar>(250)),"Height scaling")
