...
- Schreibe Klasse, die Input-Events auf Fahrzeugaktionen umleitet
  (Steering ändert etc.)
- Hierzu muss ein Vehikel eine Funktion "set_steering" bekommen, die
  das Steering als Bogenmaß oder so setzt. Mal gucken was bullet dafür
  nimmt.
- Außerdem brauch ein Vehikel eine Funktion "set_force", die am besten
  ein float in [0,1] bekommt und das anteilig an der maximum-Force
  macht.
- Dasselbe beim Bremsen.
- Der Fahrzeugtest bekommt als Kommandozeilenparameter eine
  json-Datei, die dann geladen wird.
- Man kann die Kamera (in einer festlegbaren Entfernung) an ein festes
  oder sich bewegendes Objekt binden. Das läuft so: Es gibt eine
  globale Funktion "lock_camera_to". Diese bekommt:
	- die Kamera und
	- ein gizmo (forward,right,up), also ein Koordinatenkreuz
	- eine Position p
	- zwei Abstände, d_forward und d_up
  Die Kamera bewegt sich dann von p ausgehend -d_forward nach "hinten"
  und dann d_up nach "oben". Es wird dann natürlich das lokale
  Koordinatensystem des Fahrzeugs übergeben (wie kommt man da ran?)
- Danach Nebel einbauen
- Dann Fahrsounds einbauen (bzw. allgemein Sounds)
- Dann Spielmechanik implementieren (einsammeln von Checkpoints) und
  somit erste Version des Spiels komplettieren
- Danach Shadowmaps einbauen
- Dann Hindernisse

[[https://docs.google.com/Doc?docid=0AXVUZ5xw6XpKZGNuZG56a3FfMzU0Z2NyZnF4Zmo&hl=e
n][Vehicles in Bullet]]
...
packbar sein.
*** TODO Schreibe Klasse "input_delegator", die die Eingabe delegieren kann
