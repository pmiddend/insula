...
** bullet
1. kdevelop neu installieren
2. bullet in kdevelop öffnen
3. HelloWorld verstehen, dazu Manual lesen
4. 3D-Physikbeispiel in sge nachbauen (Ball, der auf schiefe Ebene fällt, oder so)
	 Mathematica.

1. btCollisionObject hat eine Shape und ein Transform
2. Eine Shape kann von mehreren btCollisionObjects benutzt
   werden. D.h. ein Fahrzeug oder ein Baum würde nur _einmal_ als Shape
   modelliert und dann nur weitergegeben.
3. btCollisionWorld enthält btCollisionObjects (btDiscreteDynamicsWorld
   ist davon abgeleitet)
4. btPersistentManifold ist ein Cache für ein Paar von Objekten. Es
   werden Kontaktpunkt gespeichert.
5. Für Terrain: btHeightfieldTerrainShape als Shape.
6. Für Auto: Dynamics/Vehicle/*?
7. btMotionState überladen
8. [[https://docs.google.com/Doc?docid=0AXVUZ5xw6XpKZGNuZG56a3FfMzU0Z2NyZnF4Zmo&hl=en][Vehicles in Bullet]]

- Erstelle Klasse physics::vehicle. 
- Die kriegt ein Model "chassis" im Konstruktor, was zum Chassis wird
  (als Boxshape). D.h. man muss sich die Boundingbox merken. 
- Zudem kriegt die ein Model "wheel", was für die Räder eingesetzt wird.
- Es kriegt dazu noch einen Container aus "wheel_infos". Dazu gehört:
  - Relative Position des Zentrums des Wheels
	- Ein Flag, ob es ein Vorderrad ist.
	- Suspension-Stiffness
  - Damping Relaxation
  - DampingCompression
  - Frictionslip
  - Rollinfluence
	- Maxium Suspension Force (erstmal nicht beachten)
	- Gets Engine Force
  - Gets Breaking Force
  - Gets Steering
- Es kriegt ein Gewicht

- Schreibe Klasse, die Input-Events auf Fahrzeugaktionen umleitet
  (Steering ändert etc.)
...
** Vorgehen der Reihe nach [1/9]
1. [X] Gucken ob es Beispiele zu bullet mit Terrain gibt 
2. [ ] md3-Loader von sge ausprobieren, ein Model laden
3. [ ] Das Model in einem separaten Programm anzeigen lassen
4. [ ] bullet-Beispiele zu Fahrzeugen genau studieren, dabei drauf
   achten, wie man Models in Bullet integriert
5. [ ] Probieren, ein md2-Model mit einem bullet-Model zu verbinden,
   sodass man z.B. einen Klotz über die Landschaft purzeln lassen kann
6. [ ] Ein Fahrzeug auf die Landschaft tun
7. [ ] Die Kamera von hinten oben auf das Fahrzeug richten, darauf
   achten dass der Abstand konstant ist.
8. [ ] Das Fahrzeug mit den Pfeiltasten steuerbar machen
...
-Boundingsphere
-Exact
*** TODO Die Fahrzeugeigenschaften sollten in einer json-Datei zusammengefasst werden
...
*** TODO Das Terrain muss flipQuadEdges haben
		 CLOSED: [2010-08-08 Sun 23:10]
		 CLOSED: [2010-08-03 Tue 20:44
