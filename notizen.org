...
- Normaleninterpolation einbauen
	- Man braucht an jedem diskreten Punkt die Normale.
	- Für die Normale an einem Punkt benötigt man jeden diskreten Punkt.
	- Für =height_for_point= benötigt man das Dreieck zu einem
    nichtdiskreten Punkt
	- Für den Raycast braucht man das Dreieck und die Normalen zu einem
    nichtdiskreten Punkt
	-> Lösung: height_map::object speichert ein Array von Punkten und
             ein Array von Normalen. Zu einem nichtdiskreten Punkt kann man sich
						 das Dreieck mit Punkten und normalen zurückgeben lassen.
